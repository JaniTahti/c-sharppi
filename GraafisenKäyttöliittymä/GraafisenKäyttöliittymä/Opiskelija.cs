...
{
    MySqlCommand komento = new MySqlCommand();
    String paivityskysely = "UPDATE `yhteystiedot` SET `Etunimi`= @enm, " + "* Sukunimi= @snm, puhelin`= @puh,`sahkoposti`= @eml, opiskelijanumero`= eono" + " WHERE oid = @oid";
    ...
    if (komento.ExecuteNonQuery() == 1)
     ...
     else
     {
        ...
        public bool poista0piskelija(String ktunnus)
        {
            MySqlCommand komento = new MySqlCommand();
